...
class MinimumDistance(ModelInterface):
    def __init__(self, df, class_column='Species', columns_ignored=-1):
        ...
        for class_label in self.classes:
            avg_dict, avg_list = get_averages(
                df_copy[df_copy[class_column] == class_label])
    ...
    def get_equation(self):
        ...
        squared_sum = sum(
            x ** 2 for x in self.averages_list[1]) / len(self.averages_list[1])
        # You can adjust the decimal places as needed
        result = round(squared_sum, 2)
