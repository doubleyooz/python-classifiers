...
class NaiveBayes(ModelInterface):
    def __init__(self,  df, class_column='Species', columns_ignored=-1, gaussian=True):
        ...
        self.means = {cl: {feat_name: None for feat_name in self.columns}
                      for cl in self.classes}
        self.gaussian = gaussian
        self.class_mapping = {class_label: idx for idx,
                              class_label in enumerate(self.classes)}

        self.std = {cl: {feat_name: None for feat_name in self.columns}
                    for cl in self.classes}
        # number_of_classes = len(class_labels)
        self.prior = []

        for cl in self.classes:
            # it extracts all the datapoints where the Y value is the given cl
            df_class = df_copy[df_copy[class_column] == cl]
            # calculate the prior probability for each class. We're dividing the number of samples where Y = y by the total of samples
            self.prior.append(len(df_class)/len(df_copy))
            self.df[cl] = df_class
    ...
    def decision_function(self, x):
        ...
        for j in range(number_of_classes):  # loop over every class
            ...
            for idx, i in enumerate(self.columns):   # loop over every feature
                ...
                if self.gaussian:
                    likelihood[j] *= self._calculate_likelihood_gaussian(
                        i, x[idx], self.classes[j])
                else:
                    likelihood[j] *= self.calculate_likelihood_categorically(
                        i, x[idx], self.classes[j])
    ...
    def calculate_likelihood_categorically(self, feat_name, feat_value,  class_value):
        ...
        p_x_given_y = len(
            df_class[df_class[feat_name] == feat_value]) / len(df_class)
        return p_x_given_y
    ...
    def _naive_bayes_gaussian(self, X):
        ...
        for x in X:
            ...
            # add to the returning array the class where the posterior probability is the highest
