...
class Perceptron(ModelInterface):
    ...
    def fit(self, inputs, targets, learning_rate=0.01, epochs=10000, plateau_tolerance=20, reset_weights=False, verbose=False):
        ...
        while current_interaction < epochs and plateau < len(inputs) - 2:
            ...
            for index, x_i in enumerate(inputs):
                ...
                if plateau >= plateau_tolerance and learning_rate > min_lr:
                    ...
                    learning_rate *= 0.5
                    if verbose:
                        print(f'update: {update}, plateau: {
                              plateau}, learning_rate: {learning_rate}')
                    plateau = 0

                elif disturbance >= plateau_tolerance/2 and learning_rate < max_lr:
                    learning_rate *= 1.5
                    if verbose:
                        print(f'update: {update},  disturbance: {
                              disturbance}, learning_rate: {learning_rate}')
                    disturbance = 0
        ...
        print(f'final learning_rate: {learning_rate}, plateau: {
              plateau}, disturbance: {disturbance}')
    ...
    def get_equation(self):
        ...
        weights_str = " + ".join([f"{round(w, 2)} * x{i+1}" for i,
                                 w in enumerate(self.weights)])
        bias_str = f"- {round(self.weights[-1], 2)
                        }" if self.weights[-1] != 0 else ""

        return f"Decision Boundary Equation: {weights_str} {bias_str}"

    def predict(self, point):
        row = {col: point[point_name]
               for col, point_name in zip(self.columns, self.point_names)}
