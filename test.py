...
def use_classifier (data2, classifier: ModelInterface, pairs = ['setosa', 'versicolor'], given_point=None):
    ...
    # print(modified_df[columns[4]])
    modified_df[columns[5]] = modified_df.apply(classifier.classify, axis=1)

    # original_df[columns[4]] = modified_df.apply(classifier.classify, axis=1)
    ...
    # print(modified_df)
    # print(modified_df[modified_df[columns[4]] != modified_df[columns[5]]])
    ...
    decision_function_values = classifier.get_decision_values2(values_grid, columns)
...
def plot_cm(data2, classifier, columns):
    data_df = data2.copy()

    # Apply the classifier to the test set
    data_df['Predicted Species'] = data_df.apply(classifier.classify, axis=1)

    #print(data_df[data_df['Species'] != data_df['Predicted Species']])
    print(data_df)
    # Create a confusion matrix
    cm = confusion_matrix(data_df['Species'], data_df['Predicted Species'], labels=columns)
